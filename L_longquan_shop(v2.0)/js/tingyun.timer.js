	...
	$.fn.datetimepicker = function( opt ) {
		var KEY0 = 48,
			...
			createDateTimePicker = function( input ) {
				var datetimepicker = $('<div '+(options.id?'id="'+options.id+'"':'')+' '+(options.style?'style="display:none;'+options.style+'"':'style="display:none"')+' class="xdsoft_datetimepicker xdsoft_noselect '+options.className+'"></div>'),
				...
				datetimepicker
					.on('xchange.xdsoft',function( event ) {
						...
						table+='</tbody></table><a style="margin-top:5px" tingyun="clear">清空选中</a>';
				...
				calendar.on('click.xdsoft','a[tingyun="clear"]',function(){
...
$().ready(function(e) {
	$('.inp_timer').each(function() {
		if($(this).hasClass('w100')) $(this).datetimepicker({timepicker:true});
		else $(this).datetimepicker({timepicker:false});
	});
	$("<link>").attr({rel:"stylesheet",type:"text/css",href:"/com/css/tingyun.timer.css"}).appendTo("head");
});